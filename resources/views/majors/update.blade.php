<x-default-layout title="Edit Major" sectionTitle="Edit Major">
    @if ($errors->any())
    ...
    <form action="{{ route('majors.update', $major->id) }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $major->name) }}" class="w-full border border-zinc-300 rounded px-3 py-2">
        </div>
        <div>
            <label class="block mb-1 font-medium">Code</label>
            <input type="text" name="code" value="{{ old('code', $major->code) }}" class="w-full border border-zinc-300 rounded px-3 py-2">
        </div>
        <div>
            <label class="block mb-1 font-medium">Description</label>
            <textarea name="description" class="w-full border border-zinc-300 rounded px-3 py-2">{{ old('description', $major->description) }}</textarea>
        </div>
