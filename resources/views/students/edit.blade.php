        ...
        <form action="{{ route('students.update', $student->id) }}" method="POST"
                ...
                <div>
                    <div class="flex flex-col gap-2">
                        ...
                        <input type="text" id="name" name="name"
                            class="px-3 py-2 border border-zinc-300 bg-slate-50 rounded"
                            placeholder="Student Name" value="{{ old('name', $student->name) }}">
                        @error('name')
                ...
                <div>
                    <div class="flex flex-col gap-2">
                        ...
                        <input type="text" id="student_id_number" name="student_id_number"
                            class="px-3 py-2 border border-zinc-300 bg-slate-50 rounded"
                            placeholder="Student ID (e.g., F55122001)" value="{{ old('student_id_number', $student->student_id_number) }}">
                ...
                <div>
                    <div class="flex flex-col gap-2">
                        ...
                        <input type="text" id="phone_number" name="phone_number"
                            class="px-3 py-2 border border-zinc-300 bg-slate-50 rounded"
                            placeholder="Phone number (e.g., 081234567890)" value="{{ old('phone_number', $student->phone_number) }}">
                        @error('phone_number')
                        <div class="text-red-500">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="flex flex-col gap-2">
                ...
                <select name="gender" id="gender"
                    class="px-3 py-2 border border-zinc-300 appearance-none bg-slate-50 rounded">
            ...
            <div class="flex flex-col gap-2">
                    ...
                    <select name="majors" id="majors"
                        ...
                        <option value="{{ $major->id }}" {{ old('majors', $student->majors_id) == $major->id ? 'selected' : '' }}>
                            {{ $major->name }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <select name="status" id="status"
                        class="px-3 py-2 border border-zinc-300 appearance-none bg-slate-50 rounded">
                        <option value="" disabled {{ old('status', $student->status) == '' ? 'selected' : '' }}>Select Student Status</option>
